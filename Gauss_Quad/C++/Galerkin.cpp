...
int main(){
	...
	//DEFINING GAUSS PARAMETERS AND SHAPE FUNCTIONS
	int ngaus = 2;
	int weight[2] = {1,1};
	double xi[2] = {-0.57735,0.57735};
	double N1[2] = {(1.0-xi[0])/2.0,(1.0-xi[1])/2.0};
	double N2[2] = {(1.0+xi[0])/2.0,(1.0+xi[1])/2.0};
	double dN1[2] = {-0.5,-0.5};
	double dN2[2] = {0.5,0.5};
	...
	switch (choice){
		...
		str = "Galerkin";
			for (i=0;i<Nelem;i++){		//LOOP OVER ELEMENTS
				...
				for (j=0;j<ngaus;j++){	//LOOP OVER GAUSS POINTS
					K_i[0][0] += weight[j]*(a*N1[j]*dN1[j] + nu*dN1[j]*dN1[j]*2/h);
					K_i[0][1] += weight[j]*(a*N1[j]*dN2[j] + nu*dN1[j]*dN2[j]*2/h);
					K_i[1][0] += weight[j]*(a*N2[j]*dN1[j] + nu*dN2[j]*dN1[j]*2/h);
					K_i[1][1] += weight[j]*(a*N2[j]*dN2[j] + nu*dN2[j]*dN2[j]*2/h);
		...
		str = "Petrov-Galerkin";
			for (i=0;i<Nelem;i++){		//LOOP OVER ELEMENTS
				...
				for (j=0;j<ngaus;j++){	//LOOP OVER GAUSS POINTS
					K_i[0][0] += weight[j]*(a*N1[j]*dN1[j] + nu*dN1[j]*dN1[j]*2/h + a*alpha*dN1[j]*dN1[j]);
					K_i[0][1] += weight[j]*(a*N1[j]*dN2[j] + nu*dN1[j]*dN2[j]*2/h + a*alpha*dN1[j]*dN2[j]);
					K_i[1][0] += weight[j]*(a*N2[j]*dN1[j] + nu*dN2[j]*dN1[j]*2/h + a*alpha*dN2[j]*dN1[j]);
					K_i[1][1] += weight[j]*(a*N2[j]*dN2[j] + nu*dN2[j]*dN2[j]*2/h + a*alpha*dN2[j]*dN2[j]);
				}						//GLOBAL ASSEMBLY OF K MATRIX
				K[T[0]][T[0]] += K_i[0][0];
				K[T[0]][T[1]] += K_i[0][1];
				K[T[1]][T[0]] += K_i[1][0];
	...
	*/
	...
	output.close();
	cout << "Results have been generated";
